...
\weight_T(q, au, bv, q') & = & 
   ...
   \wei_{00}(q, a, b, r) \otimes \weight_T(r, au, bv, q')\\
  & \oplus & \displaystyle\bigoplus_{r \in Q, u \neq \epsilon} &
    \wei_{10}(q, a, b, r) \otimes \weight_T(r, u, bv, q')\\
  & \oplus & \displaystyle\bigoplus_{r \in Q, v \neq \epsilon} &
    \wei_{01}(q, a, b, r) \otimes \weight_T(r, au, v, q')\\
  & \oplus & \displaystyle\bigoplus_{r \in Q, u,v \neq \epsilon} &
    \wei_{11}(q, a, b, r) \otimes \weight_T(r, u, v, q')\\
...
% 
The above sum may be infinite because of the transitions $\wei_{00}$. % in the first and second lines.
...
increment the current position in the input word if if $i = 1$ (otherwise, do not change it),
increment the current position in the output word if if $j = 1$, 
and change state from $q$ to $q'$.
%
In contrast with the models of weighted transducers over finite alphabets~\cite{Mohri03ijfcs},
the input and output symbols at current positions are read by every transitions, 
even transitions that do not increment the head's position.
...
This is an important feature in the case of an infinite alphabet, 
since symbols cannot be stored in the finite memory of the transducer in this case.
...
\medskip\noindent
Let $\< s, t> \in \Sigma^+ \times \Delta^+$, with $s = s_0\ldots s_m$, $t = t_0\ldots t_n$). 
The weight associated by $T$ to $\< s, t>$
is defined as follows:
\begin{equation}
T(s, t)  = 
\displaystyle\bigoplus_{q, q' \in Q} \mathsf{in}(q, s_0, t_0) 
\mathop{\otimes} \weight_T(q, s, t, q') \mathop{\otimes} \mathsf{out}(q', s_m, t_n)
\label{eq:SWT-value}
\end{equation}
...
and the computation of $A$ on words $s \in \Sigma^+$
...
\weight_A(q, au, q') & = & 
   \displaystyle\bigoplus_{r \in Q} &
   \wei_{0}(q, a, r) \otimes \weight_A(r, au, q')\\
  & \oplus & \displaystyle\bigoplus_{r \in Q, u \neq \epsilon} &
...
$s = s_0\ldots s_m \in \Sigma^+$ is: % defined as follows:
\begin{equation}
A(s)  = 
\displaystyle\bigoplus_{q, q' \in Q} \mathsf{in}(q, s_0) 
\mathop{\otimes} \weight_A(q, s, q') \mathop{\otimes} \mathsf{out}(q', s_m)
\label{eq:SWA-value}
\end{equation}
