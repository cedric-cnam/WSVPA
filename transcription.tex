...
Moreover, it contains a time value $\ioi{e}$ (real number) in RTU (seconds)
...
and an IOI duration value expressed in MTU (rational number).
%The alphabet $\Delta$ 
...
\textsf{qparse} based on a similar approach.

slightly different models of symbolic weighted tree automata
...WTA  specification of a prior score language.

% we hope to gain flexibility with the present framework and extend application field
% from transicription to other problems related to music notation and symbolic recorded performances.
