%
% transducers and automata with epsilon transitions
%
...
\[
\begin{array}{rccl}
\weight_T(q, \epsilon, \epsilon, q') & = &              \wei(q, \epsilon, \epsilon, q')\\
\weight_T(q, s, t, q') & = & 
...
% impossible, needs reading input/output symbols even by epsilon-transitions.
...
\wei_{10}(q_0, d, \epsilon, q_1) & = & \alpha\\
\end{array}
\]
%
Finally, it may happen that the performers plays an extra note accidentally, but only once in a row. 
This is modelled by the transition $\wei_{10}$ with an arbitrary weight value $\alpha \in \Semiring$, 
switch from state $q_0$ (normal) to $q_1$ (error).
...
the transition functions $\wei_\epsilon$ and $\wei_\Sigma$
