...
\begin{example}\label{rules-division}
The recursive subdivision of time that leads to hierarchichal
structures of music notation can be modeled as
production rules. Since there exists several possible division,
rules can be weighted
in the tropical semiring whose domain $\mathbb{R}_+ \cup \{ +\infty\}$,
$\oplus$ is min, $\zero = +\infty$, $\otimes$ is sum, and $\one = 0$.
For instance, the following production rules define two possible divisions
of a bounded time interval into respectively a duplet and a triplet.
\[
