...
\begin{definition}
A \emph{weighted transducer} $T$ over the input and output alphabet $\Sigma$ and $\Delta$ 
...
\mathop{\otimes} \weight_\A(q, s, t, q') \mathop{\otimes} \mathsf{out}(q').
\]

A \emph{weighted automata} $T = \< Q, \init, \weight, \final >$
over $\Sigma$ and $\Semiring$ 
is defined in a similar way by simply omitting the output symbols,
\ie $\weight$ is a function of~$Q \times Q$ into %$\Phi$.
$\< \Phi_0, \Phi_\Sigma >$, 
