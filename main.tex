...
principle of quantitative parsing / parsing of words over infinite alphabet:
...
\noindent
\quad\textbf{extract} from $P$ $\< q_1, \sigma, q_2>$ with a minimal $d(q_1, \sigma, q_2)$ wrt $\leq_\oplus$.
...
\displaystyle\bigoplus_{{\return{r}} \in \Sigmar} \phi({\call{c}}, {\return{r}})\\
\mathrm{and} &  d(q_0, \bot, q_3) & \opluseq &
d(q_1, \sigma, q_2) \otimes
%\displaystyle\bigoplus_{p \in P} 
\displaystyle\bigoplus_{{\call{c}} \in \Sigmac} 
\displaystyle\bigoplus_{{\return{r}} \in \Sigmar} \phi({\call{c}}, {\return{r}})\\
& & & \mathrm{where~} \phi = \weic(q_0, q_1, p) \otimes \weir(q_2, p, q_3)\\
%
...
%
%  & d(q_1, \sigma, q_3) & \opluseq &
%  d(q_1, \sigma, q_2) \otimes d(q_2, \sigma, q_3)
\end{array}
\) 
\end{algo}
...
A music score is represented as a structured word
