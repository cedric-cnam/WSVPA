...
(e.g., MIDI pitch 69 and note $\mbox{\footnotesize A4}$),
...
It associates to every nested words over infinite alphabets a weight value.
...
%rest symbol R.
$\Deltac$ and $\Deltar$ contain respectively opening and closing parentheses.
...
i.e., an ornemental note with theoretical duration zero.
\endex
\end{example}
...
We consider a \SWVPA over the alphabet of Ex.~\ref{ex:nested-word}
...
\philippe{Je trouve l'exemple un peu confus. Je ne sais pas
quel est "l'associate time interval" d'un symbole. Je ne sais
pas à quoi fait référence "the following call transition". Je
n'ose pas trop toucher mais je crois qu'il faut revoir.}
...
%
Algorithm~\ref{algo:Dijkstra}
terminates in PTIME and at termination, $d_\bot = b_\bot$ and $d_\top = b_\top$.
\florent{(see Appendix~\ref{sec:bestsearch}).}
With~\eqref{eq:min}, this ensures
its correctness. %of Algorithm~\ref{algo:Dijkstra}.
%
\begin{figure}[ht]
...
%\florent{REVOIR}
Thanks to the hypothesis of effectiveness of $\bar\Phi$, 
it is possible to construct suring the iteration a witness word
for Proposition~\ref{th:best-search}, 
\ie a word of $\Delta^*$ with a minimal weight \wrt $A$.
