...
% 
\title{Symbolic Parsing and\\ Automated Music Transcription}
%\title{Weighted Visibly Pushdown Automata\\ Automated Music Transcription}
...
In such a model, a weight value is associated to each transition rule, % production rule
...
is used to handle ambiguity of the model, 
...
** DEL ** 
%Parsing produces an analyze of an input word in the form of a parse tree 
%representing the derivation of a grammar.
In case of ambiguity, several analyses, in general an exponential number, 
are possible for the same input word. 
Weighted CFG associate a weight value to each of its derivation trees, 
based on weighted values associated to each production rules, 
and are therefore useful for the compact representation and the ranking of large sets of parse trees.
Moreover, there exist some efficient specialized parsing algorithms~\cite{Huang05kbest}
to compute the $n$ best derivations of a given input word.

The models and algorithms mentioned above deal with finite input alphabets.
Considering infinite alphabets can however be of practical interest, 
for instance when dealing with sequences of timestamped events.
(approach of music transcription by parsing~\cite{MCM19})


%generalizes weighted parsing: 
%finding the best derivation for a weighted grammar.

We propose here some models of weighted recognizers 
...
and the weight associated to the transition rule is a function of these variables.
%
This approach is close to the one of Symbolic Automata (SA)~\cite{VeanesdAntoniJACM}, 
except that the domain of weight values 
...
The models defined here are finite automata, transducers and pushdown automata with visibly restriction.
%wrt an edit distance
The latter model of these automata computes on nested words~\cite{AlurMadhusudan09nested}, 
a structured format of words parenthesized with markup symbols, 
which can correspond to a linearization of trees.

The latter corresponds in expressiveness
to a strict restricted model of weighted grammars
...
and weighted transducers handling infinite alphabets. 
