...
\).
\noindent
We described earlier the special cases where the stack is empty, which applies for example when reading the first measure for which we only push $<\ccall{\mathsf{m}}\tstamp{[0, 1]}, q_{1/1}>$ in the stack:
\(
\weiec\bigl(q_{1/1},
