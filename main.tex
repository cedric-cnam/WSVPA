...
\title{Symbolic Weighted Language Models,
       Quantitative Parsing and Verification\\ over Infinite Alphabets}
...
%
The models presented in this section are weighted automata 
and transducers~\cite{Droste09handbook}
%over finite alphabets, see \eg~\cite{Mohri03ijfcs},
with transitions labeled by weight functions
that take the input and output symbols as parameters.
...
%
These models are also particular cases of the very general model of 
Weighted Symbolic Automata with Data Storage~\cite{Herrmann16dlt}. 
%\florent{review 3: = classical transducers}
...
\begin{definition}\label{def:transducer} \label{def:SWT}
...
%
Note that the SW-transducers of Definition~\ref{def:transducer} %do not form a new model, 
are actually classical weighted transducers where the alphabets are permitted to be infinite.
%
\noindent
...
%
Since~$\zero$ is absorbing for~$\otimes$, 
and neutral for $\oplus$ in~$\Semiring$,
if $\wei_{ij}(q, a, b, q'')$ is equal to $\zero$
(meaning that there is no possible transition
from state $q$ into state $q'$ while reading $a$ and $b$), 
then the entire term containing this expression can be ignored in the sum.
...
type of data storage, and 
associates to every nested word over an infinite alphabet a weight value
in a semiring.
...
we aim at choosing the simplest one, \ie the one with the smallest weight.

\lydia{We should find a better variable name than j... it does not render well in fractions}
Let us assume that the top of the stack is a tuple 
made of the call symbol~$\ccall{n}\tstamp{[\tau, \tau+\ell]}$ 
and the state~$q$. 
Let us now consider a new call transition starting in state $q_{\frac{j}{n}}$, that is the $j$-th state of the $n$ sub-intervals of duration~$\frac{\ell}{n}$.
...
The pair $\< c', p'>$ must be popped later, during the computation from $q$ to $q'$,
...
\medskip
\input{algo-best-alt}
%\input{algo-best} % former version
...
(it is however uncomparable to 
 related problem called~\emph{semiring parsing},
 \cite{Goodman99SemiringParsing},
  called~\emph{weighted parsing}~\cite{MorbitzVogler19weighted-parsing}).
%
The interested reader might find in Appendix~\ref{sec:trees} 
more details on the correspondence between 
...
We presented three properties of three Symbolic Weighted language models
(automata, transducers and visibly pushdown automata), %over infinite alphabets,
and applied them to the problem of parsing
with infinitely many possible input symbols (typically timed events).
One originality of this approach 
%extends conventional parsing and weighted parsing
is the comparison of words using 
a distance between words defined by a given SW transducer.
This allows to consider finer word relationships than strict equality.

This work can be extended in several directions. 
The best-search algorithm for \SWVPA
could be generalized from $1$-best to $n$-best~\cite{Huang05kbest},
and to $k$-\emph{closed} semirings~\cite{Mohri02semiring}
(instead of \emph{bounded}, which corresponds to $0$-\emph{closed}). 
It complexity upper bound could also certainly be improved.
...
Another possible study could be the existence of best-search algorithms
for the more general models of~\cite{Herrmann16dlt}.

\noindent
Finally, the  best-search algorithm presented here works offline, 
whereas an on-the-fly approach coupling automata construction and best-search 
would be interesting \eg for online XML validation or filtering, 
or program monitoring~\cite{dAntonyAlur14SVPDA}.
...
\input{app-Bar-Hillel-Perles-Shamir}
...
\section{Proof of Proposition~\ref{algo-shortest}}\label{app:shortpath} % Shortest path
\input{algo-best-lemma.tex}
