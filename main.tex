...
The various approaches to weighted parsing usually assume a finite input alphabet. %models and algorithms 
Considering infinite or large alphabets can however be of practical interest, 
for instance when processing strings in UTF encodings or for vulnerability detection in Web-applications 
(see \eg~\cite{dAntoni21CACM}), for model checking, 
or when dealing with sequences of timestamped events
(like in the approach of~\cite{MCM19} of automated music transcription by parsing).
...
%
This approach is close to the case of Symbolic Automata (SA)~\cite{VeanesdAntoniJACM}, 
