...
the problem
%quantitative parsing or symbolic parsing
of parsing of words over infinite input alphabet.

A weighted CFG associates a weight value to each of its derivation trees.
There exists some specialized parsing algorithms~\cite{nbest} 
to compute the $n$ best derivations of a given input word.

Such models and algorithm only deal with finite input alphabets.
...
...example of music transcription\dots

%generalizes weighted parsing: 
%finding the best derivation for a weighted grammar.

We propose here models of weighted recognizers 
model of weighted CFG over infinite input alphabets %sets of terminal symbol
(but with finite sets of states and transitions rules). 
% non-terminals and production rules).
In the production rules, input symbols appear as variables (parameters)
and the weight associated to the transition rule is a function of these variables.
%
It is the approach of Symbolic Automata~\cite{VeanesdAntoniJACM}, 
...
models: automata, transducer and pushdown automata with visibly restriction.
%wrt an edit distance
The latter corresponds in expressiveness
to a strict restricted model of weighted grammars. %VPA 
That permits to define a general approach for parsing 
over infinite input alphabets.
