...
plus the total duration of intervals read so far, $i \ell$.
...
We can penalize triplets compared to duplets with $\alpha_2 < \alpha_3$.
%
\noindent 
Along with $\ccall{d}$, 
...
** case of new measure symbol: $q_0$ is a special (initial) state.
first measure:
\(
\weiec\bigl(q_0, 
           \ccall{\mathsf{m}}\ts{0}\ts{1}, q_0, 
           q_0 \bigr) = \alpha_d
\).
new measure:
\(
\weic\bigl(q_{{c}/{c}}, 
           \ccall{\mathsf{m}}\ts{\tau}\ts{1}, q_0, 
           \ccall{\mathsf{m}}\ts{\tau + 1}\ts{1}, q_0,
           q_0 \bigr) = \alpha_d
\).
\endex
\end{example}
