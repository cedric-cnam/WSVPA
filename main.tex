...
of structuring an input linear representation %typically
...
%
for the analyse of data streams or serialization of structured documents~\cite{Segoufin06csl}, 
or processing of timed execution traces.
Regarding the latter case, we describe briefly at the end of the paper 
a parse-based approach to automated music transcription,
where a symbolic music performance (sequence of timed musical events) % timestamped
...
Various extensions of automata have been studied for handling infinite alphabets.
%words carrying data values in an infinite domain (e.g. integers) e.g. data processing 
To cite a few, some memory extensions allow restricted storage and comparison of input symbols, 
%and correspond with logics, 
(see~\cite{Segoufin06csl} for a survey)
like pebbles for marking positions~\cite{NevenSchwentickVianu04FSMinfinite}, 
registers~\cite{KaminskiFrancez94}, 
computations in 2 steps~\cite{Bojanczyk11FO2}... % data words automata.
%
...
similar to representation of automata in model checkers, 
following a similar idea, 
Symbolic automata, 
transitions guarded by predicates over alphabet theories.
 

another quantitative issue in parsing is non-determinism : 
with an ambiguous grammar, there may exists several derivations yielding one input word.
%the problem: quantitative parsing or symbolic parsing
%of parsing of words over infinite input alphabet.
selection by ranking derivation according to an associated weight value...
Weighted parsing~\cite{Goodman99SemiringParsing,Nederhof03weightedParsing,MorbitzVogler19weighted-parsing} 
consists in analysing a word in input \wrt a weighted language model, \eg a probabilistic context-free grammar (CFG).
