...
$\alpha, \alpha' \in \Phi_\epsilon$,
all $\phi, \phi' \in \Phi_\Sigma$,
all $\psi, \psi' \in \Phi_\Delta$, 
and $\eta, \eta' \in \Phi_{\Sigma, \Delta}$, the function
\begin{description}
\item $\phi \otimes \alpha : x \mapsto \phi(x) \otimes \alpha$, 
      and $\alpha \otimes \phi' : x \mapsto \alpha \otimes \phi'(x)$ belong to $\Phi_\Sigma$, 
...
\noindent
\marginpar{necessary?}
Finally, it is also required that the codomain of every function of $\Phi_\Sigma$ and $\Phi_\Delta$
is a subset of $\Phi_\epsilon$,
and that all partial applications of functions $\Phi_{\Sigma, \Delta}$, 
...
and $\vec{\wei}$ is a tuplet made of the 4 of transition functions 
$\wei_\epsilon$, $\wei_\Sigma$, $\wei_\Delta$, and $\wei_{\Sigma, \Delta}$
from $Q \times Q$ into respectively
$\Phi_\epsilon$, $\Phi_\Sigma$, $\Phi_\Delta$, and $\Phi_{\Sigma, \Delta}$.
\end{definition}
%
We synthesize the above 4-uplet of transition functions into a unique 
function~$\wei$ from
...
and the semiring $\Semiring$ is a tuple
$A = \< Q, \init, \vec{\wei}, \final >$,
where $Q$ is a finite set of states, 
$\mathsf{in} : Q \to \Semiring$, 
respectively $\mathsf{out} : Q \to \Semiring,$
are functions defining the weight for entering, 
respectively leaving, a state, 
and $\vec{\wei}$ is a tuplet made of the 2 of transition functions 
$\wei_\epsilon$ and $\wei_\Sigma$ from $Q \times Q$ into respectively
$\Phi_\epsilon$ and $\Phi_\Sigma$.
\end{definition}
...
there exists a \SWA 
$A_{T, s}$ such that for all $t \in \Delta^*$, 
$A_{T, s}(t) = T(s, t)$.
\end{proposition}
...
% gen. to transducers
\begin{proposition}
Given a \SWA $A$ over $\Sigma$ and $\Semiring$, 
there exists a \SWA $A'$ without $\epsilon$-transitions 
...
\[
\wei'_\Sigma(q, q') = 
\displaystyle\bigoplus_{q_1 \in Q} \wei_\Sigma(q, q_1) \otimes \wei_\epsilon(q_1, q')
\oplus  \displaystyle\bigoplus_{q_2 \in Q} \wei_\epsilon(q, q_2) \otimes \wei_\Sigma(q_2, q')
\]
In the above definition of $\wei'_\Sigma$ we use the operators 
$\oplus$ and $\otimes$ on $\Phi_\Sigma$ described in Section~\ref{section:symbols}.
\qed
\end{proof}     
