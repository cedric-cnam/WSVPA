...
\newcommand{\ts}[1]{\mathop{:}{#1}} % timestamp marker
...
\newcommand{\creturn}[1]{\ensuremath \rangle_{#1}}
...
when reading  $a$ and $b$ the input and output words,
...
We build a small \SWT over the alphabets $\Sigma$ and $\Deltai$ of
Examples~\ref{ex:running} and \ref{distance-time},
...
and the subsequence
of the events of $\Deltai$ in a score.
%the performance $I$ and
%the score $O$ of Example~\ref{ex:running}.
%
When reading one performed event $\mu$
and the corresponding notated event $\nu$
(for instance MIDI pitch 69 and note name $\mbox{\footnotesize A4}$),
the weight value computed by the \SWT is the time distance between both,
(see Example~\ref{distance-time}), as modeled by transitions $\wei_{11}$ below.
%
If we meet the continuation symbol $'-'$
(this occurs for instance in \emph{ties}
$\musQuarter\!\!\!\mathrel{\raisebox{-1.5mm}{$\smile$}}\!\!\!\musEighth$,
or \emph{dots} \musQuarterDotted{}),
%that represents continuation
it is  skipped with no cost (transitions $\wei_{01}$ or weight $\one$).
%Likewise, the markups are ignored.
\[
\begin{array}{lclclcll}
\wei_{11}(q_0, \mu\ts{\tau}, \nu\ts{\tau'}, q_0) & = &
     \delta(\mu\ts{\tau}, \nu\ts{\tau'}) & \quad & %|\tau' - \tau| & \quad &
\wei_{11}(q_1, \mu\ts{\tau}, \nu\ts{\tau'}, q_0) & = &
     \delta(\mu\ts{\tau}, \nu\ts{\tau'}) &
     \mbox{if~} \nu \neq - \\ %|\tau' - \tau| & \quad &
\wei_{01}(q_0, \varepsilon, \mathsf{-}\ts{\tau'}, q_0) & = & \one & &
\wei_{01}(q_1, \varepsilon, \mathsf{-}\ts{\tau'}, q_0) & = & \one\\
\wei_{10}(q_0, \mu\ts{\tau}, \varepsilon, q_1) & = & \alpha & & %\multicolumn{3}{l}{\mathrm{for~all~} b \in \Delta}
\end{array}
\]
%
...
The weight for reading of the input is obtained with
$\bigoplus^1_\Sigma$.
%
...
Consider once more the linearized score of Example~\ref{ex:running},
with $\Deltai$ representing timed notes and continuations.
%the set of music events of the form $<\nu, \tau>$
%where $\nu$ is either a note (e.g. A4), the continuation symbol $-$, or a
%rest symbol R.
The subset $\Delta \setminus \Deltai$ is partitioned into
$\Deltac$ and $\Deltar$, containing respectively opening and closing parentheses.
...
The score representation $O$ in Example~\ref{ex:running}
% score: \includegraphics[scale=0.35,trim=0 5mm 0 0]{pictures/ex1.pdf},
is a nested-word of~$\Delta^*$.
%
Another example is the second candidate
...
more precisely to its structural complexity:
given a set of equivalent representations, we aim at choosing the simpler one.

Let us assume a third component in the symbols of $\Deltac$
which is the duration of the associated time interval.
It is used to check the timestamps in the symbols.
%
For instance, the following call transition starts in state
$q_{{i}/{c}}$, meaning that the current interval has number~$i$
amongst $c$ sub-intervals of duration~$\ell$ (as indicated by the stack top).
It reads a new time-division symbol~$\ccall{d}$.
The date for this symbol has to be the time $\tau$
for the previous time-division symbol read~$\ccall{c}$ (found on the stack)
plus the total duration of intervals read so far, $i \ell$, and
the new current interval duration is~$\frac{\ell}{d}$:
%
\(
\weic\bigl(q_{{i}/{c}},
           \ccall{c}\ts{\tau}\ts{\ell}, q,
           \ccall{d}\ts{\tau + i\ell}\ts{\frac{\ell}{d}}, q_{{i+1}/{c}},
           q_{{0}/{d}} \bigr) = \alpha_d
\).

\noindent
The weight of this transition is $\alpha_d$.
...
to start the next sub-interval after reading $\creturn{d}$,
with:
\(
\weir\bigl(q_{{d}/{d}},
           \ccall{d}\ts{\tau}\ts{\ell}, q_{{i}/{c}},
           \,\creturn{d}\ts{\tau + d\ell}, q_{{i}/{c}} \bigr) = \one
\).

\noindent
Reading an event is done with:
\(
\weii\bigl(q_{{i}/{d}},
           \ccall{d}\ts{\tau}\ts{\ell}, q_{{j}/{c}},
           \mu\ts{\tau + i\ell}, q_{{i+1}/{d}} \bigr) = \alpha_\mu
\).
% cas des barres de mesure?
%For the case of a new measure symbol,
%$q_{1/1}$ is a special (initial) state.
The transition for starting to read the first measure is:
\(
\weiec\bigl(q_{1/1},
           \ccall{\mathsf{m}}\ts{0}\ts{1}, q_{1/1},
           q_{1/1} \bigr) = \one
\),
and for a new measure:
\(
\weic\bigl(q_{1/1},
           \ccall{\mathsf{m}}\ts{\tau}\ts{1}, q_{1/1},
           \ccall{\mathsf{m}}\ts{\tau + 1}\ts{1}, q_{1/1},
           q_{1/1} \bigr) = \one
\).
\endex
\end{example}
...
\end{proof}
\noindent
We now present a procedure for searching, for a~$\SWVPA$ $A$,
a word %$w \in \Delta^*$
...
\item The \SWT $T$ evaluates a ``fitness measure''  that expresses
    a correspondance between a performance and notation events. See Example~\ref{ex:SWT}.
...
(see \eg~\cite{Goodman99SemiringParsing}
 and~\cite{MorbitzVogler19weighted-parsing}).
Weighted parsing
...
We  introduced weighted language models over infinite alphabets,
...
the best search algorithm could bbe generalized from $1$-best to $n$-best~\cite{Huang05kbest},
and to $k$-\emph{closed} semirings~\cite{Mohri02semiring}
(instead of \emph{bounded}, which corresponds to $0$-\emph{closed}). Second,
we believe that
the complexity bounds of the algorithms can be more precisely characterized.
...
a suitable feature in the context of applications such as, e.g., automatic music transcription.
