...
and the second case with a root tree.
%
Every \SWTA %of Definition~\ref{def:SWTA} 
defines a mapping 
from trees of $\T(\Omega)$ into the weight values in~$\Semiring$,
based on an intermediate function
...
And in both cases, $a$ is a constraint on the label of the 
parent node in the derivation tree.
This features of observation of parent's label  
are useful in the case of infinite alphabet, 
where it is not possible to memorize a label with the states.
%
%One can observe that 
The weight of a labeled derivation tree $t$
...
\[
\begin{array}{ll}
\weii(\bar{q}, {\call{c}}, \bar{p}, a, \bar{q} q') = \wei(q', c, a, \varepsilon) & 
\mathrm{for~all~} c \in \Omega_{>0}, \bar{p}\in P', a \in \Omega_0\\
%
\weiei(\bar{q}, a, \bar{q} q') = \wei(q', \varepsilon, a, \varepsilon) & 
\mathrm{for~all~} a \in \Omega_0\\
%
\weic(\bar{q}, {\call{c}}, \bar{p}, \call{c'}, \varepsilon, \bar{q}) = \wei(q', c, c', \bar{q}) &  
\mathrm{for~all~} c, c' \in \Omega_{>0},  \bar{p}\in P'\\
%
\weiec(\bar{q}, \call{c}, \varepsilon, \bar{q}) = \wei(q', \varepsilon, c, \bar{q}) & 
\mathrm{for~all~} c \in \Omega_{>0}\\
%
%\weir: Q \times \Omegac \times P \times \Sigmar \times Q \to \Semiring & 
\weir(\bar{q}, {\call{b}}, \bar{p}, {\return{b}}, \bar{p} q') = \one & 
