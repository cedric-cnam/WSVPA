...
%
\begin{proof}
Let $T = \< Q, \init, \vec{\wei}, \final >$ 
where $\vec{\wei} \< \wei_\epsilon, \wei_\Sigma, \wei_\Delta, \wei_{\Sigma, \Delta}>$
...
%
The $\SWA$ wanted is $A_{T, s} = \< Q', \init', \vec{\wei'}, \final' >$.
\qed
\end{proof}
...
% gen. to transducers
\begin{proposition}
Given a \SWA $A$ over alphabet $\Sigma$ and a commutative and bounded semiring $\Semiring$, 
...
of a state $q \in Q$ and a stack content $\theta \in \Theta^*$, where $\Theta = \Omegac \times P$.
Therefore, $\weight_A$ is a function from 
$Q \times \Theta^* \times \Omega^* \times Q \times \Theta^*$ into $\Semiring$.
\[
\begin{array}{rcl}
...
denotes a stack where the pair made of ${\call{c}}$ and $p$ is the top symbol and $\theta$
is the rest of stack.
