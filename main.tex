...
one function $\wei_{k}: Q \times Q^{k} \to \Phi_{\Omega_k}$
for each $k \in [0..M]$. 
\end{definition}
%
...
\wei(q_0, a, q_1 \ldots q_k) & = & \phi_{k}(a) &
...
of a regular tree grammar $G$~\cite{tata}, 
...
And the above production rule can also be seen as 
a rule of a weighted CF grammar, of the form
$[a]\, q_0 := q_1 \ldots q_k$ if $k > 0$,
and $q_0 := a$ if $k = 0$. 
In the first case, $a$ is a label of the rule, 
in the second case it is a terminal symbol.
On can observe that the weight of a labeled derivation tree $t$
...
%
\weic(\bar{q}, {\call{b}}, \bar{p}, \call{b'}, \varepsilon, \bar{q}) = \one & %\wei(p', b, \bar{p}) & 
\mathrm{for~all~} b \in \Omega_{>0},  \bar{p}\in P', b, b' \in \Omega_{>0}\\
%
\weiec(\bar{q}, \call{b}, \varepsilon, \bar{q}) = \one & %\wei(p', b, \bar{p}) & 
