...
denoted $\Sigma$, $\Delta$,...
$\Sigma^*$ is the set of finite sequences (\emph{words}) over
 $\Sigma$, $\varepsilon$ the empty word, and $\Sigma^+ = \Sigma^* \\ \varepsilon$ .
...
with a music score v, we must account for
