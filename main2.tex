...
and $\weiei$ applies when the stack is empty.
...
($c$ is compared to $c'$),
and $\weiec$ applies when the stack is empty.
...
and $\weier$ applies when the stack is empty.
...
\wrt this ordering.
%
When $\sigma = \bot$, $d_0(q, \sigma, q')$ is the central expression in %a term of 
the definition (\ref{eq:weightA}) of $A(s_0)$ for the minimum $s_0$
(for the function $\weight_A$).
%
When $\sigma = \top$, intuitively, it is the minimum weight of a computation of $A$
...
as well as the second case, to push call symbols on the top of $\gamma$, 
...
that converges eventually to $d_0$.
...
\noindent\quad \textbf{for all} $q_0, q_3 \in Q$ and $p \in P$, 
update $d$ with $d(q_1, \sigma, q_2)$ (Figure~\ref{fig:best-update}).
\end{algo}

\begin{figure}[t]
\[
\begin{array}{rrcl}
\multicolumn{4}{l}{\mathrm{For~all~} q_0, q_3 \in Q, p \in P,}\\
...
\end{array}
\]
\caption{Update $d$ with $d(q_1, \sigma, q_2)$.}
...
and an alphabet $\Omega$ with a $k$-convex effective label theory,
