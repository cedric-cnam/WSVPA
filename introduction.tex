...
\florent{deplacer plus loin (application)}
...
\florent{quantitative reasoning about programs}
...
\node (SWADS) at (0,5.0) {%
  \(
  \begin{array}{c}
  \mathsf{SWADS}\,\mbox{\cite{Herrmann16dlt,Herrmann20phd}}: \Sigma_{\mathsf{inf}}^* \to \mathbb{S}
  \end{array}
  \)
};
%
...
\node (SA) at (1.8,1.7) {%8
  \(
  \begin{array}{c}
  \mathsf{SA}\,\mbox{\cite{dAntoni21CACM}}: \Sigma_{\mathsf{inf}}^* \to \mathbb{B}\\
...
\node (VPA) at (5.6,0) {%
  \(
  \mathsf{VPA}\,\cite{AlurMadhusudan09nested}
  \)
};
%
\node (WVPA) at (4.6,1.7) {%
  \(
  \mathsf{WVPA}\,\cite{Caralp12VPAmult}
  \)
};
%
\node (SVPA) at (6.6,1.7) {%
  \(
  \mathsf{SVPA}\,\cite{dAntonyAlur14SVPDA}
  \)
};
%
...
\florent{Application to parsing and tanscription. these results are appliable to other decision pb.}
