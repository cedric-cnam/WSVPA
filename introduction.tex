...
audio files, or the widely used MIDI file format~\cite{MIDIfile}.
...
\philippe{Je ne comprends pas très ce que cette première phrase fait là. Je commencerais
directement le paragraphe avec "This framework relies..."}
...
In a parsing context, a \SWVPA $A$ associates a weight value $A(t)$ % \in \Semiring$
...
We illustrate our framework with music transcription: given
a \emph{timeline} of timestamped musical events from an infinite
alphabet $\Sigma$ as input, 
parse it into a structured music score. 
Input events of $\Sigma$
are pairs $\< \mu, \tau>$, were $\mu$ is a MIDI message symbol~\cite{MIDIfile}
...
in an alphabet $\Delta$
%in Common Western Music Notation (CWMN)~\cite{Gould11Notation}
where event symbols $\nu$ are \emph{pitch names} (e.g., A4, G5, \etc.), 
and timestamp $\tau'$ belongs to a rhythmic ``grid'' obtained from recursive divisions:
...
will be used:
\begin{enumerate}
  \item A performance  $I = [ \<49, 0.07>, \<51, 0.72>, \<52, 0.91>, \<54, 1.05>, \<56, 1.36>, \<57, 1.71>]$, % over interval $[0,2[$,
  \item A score $O =$ 
  \includegraphics[scale=0.35,trim=0 5mm 0 0]{pictures/ex1.pdf},
  %\includegraphics[scale=0.20]{pictures/score5.png}, 
  corresponding to a hierarchical structures (measures, tuplets) 
  that can be linearized as a sequence of events
      $[\<\mathrm{A}4, 0>, 
        \<\mathrm{B}4, \frac{3}{4}>, 
        \<\mathrm{C}5, \frac{7}{8}>, 
        \<\mathrm{D}5, 1>, 
...
\includegraphics[scale=0.35,trim=0 5mm 0 0]{pictures/ex2.pdf}) are possible.
