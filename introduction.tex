...
Considering large or infinite alphabets can however be of practical interest, %in other cases.
%
...
\philippe{I think that we can make the case for a larger set of situations: given a linear 
music notation input, for instance elements in an XML file, we we want to structure
the input according to a hierarchical rhythmic space. I can elaborate...}
In this problem, a music performance, % a symbolic representation of a music performance 
represented symbolically in the form of a sequence of timed musical events, % timestamped
...
%(the structure represents hierarchical  grouping of events, and their relative temporal positions)
...
\philippe{This sentence (symbols as variables) is not immediatly clear to me. Maybe a short example or intuition?}
%
\begin{figure}
\centering
\begin{tikzpicture}
...
\philippe{The weight is a label on edges in the derivation tree, right? Not sure I understand 
the sentence}
%
On the other hand, 
...
\philippe{You mean the distance between $s$ and $t$? "The latter value" is a bit ambiguous}
%
...
\philippe{OK, à quelques détails (pour moi), tout ça est très bien expliqué}
