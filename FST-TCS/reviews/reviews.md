...
1. On page 5, in the label theory it is required that each set \Phi_\Sigma (and \Phi_{\Sigma,\Delta}) is recursively enumerable, in particular countable.
\Phi_\Sigma is supposed to contain all constant functions from \Sigma into the semiring S.
...
The authors should state that the symbolic-weighted transducers of Def. 7 do not form a new model. They are classical weighted transducers where the alphabets are permitted to be infinite. For this, one could simply replace their weight functions by a single function wt: Q x (\Sigma \cup \{\eps\}) x (\Delta \cup \{\eps\}) x Q \to S. In particular, the swA of Def 9 are classical weighted finite automata.